...
# Challenge name mapping by port
CHALLENGE_NAMES = {
    32104: "The Genesis Crypt",
    49205: "Echoes Of Collision",
    33428: "The Bifrost Relayer",
    37819: "The Oracle Paradox",
    35780: "The Quantum Entanglement",
}

# Get challenge name from PUBLIC_PORT environment variable
PUBLIC_PORT = int(os.getenv("PUBLIC_PORT", 0))
CHALLENGE_NAME = CHALLENGE_NAMES.get(PUBLIC_PORT, "Blockchain Launcher")
...
def get_challenge_name():
    """Get challenge name based on port number"""
    try:
        port = int(request.host.split(':')[-1])
        return CHALLENGE_NAMES.get(port, "Blockchain Launcher")
    except:
        return "Blockchain Launcher"

# Get the directory where this module is located
_MODULE_DIR = os.path.dirname(os.path.abspath(__file__))
_FRONTEND_DIR = os.path.join(_MODULE_DIR, "frontend")

@app.route("/")
def serve_frontend():
    """Serve static frontend interface with challenge name injection"""
    try:
        index_path = os.path.join(_FRONTEND_DIR, "index.html")
        with open(index_path, "r") as f:
            html = f.read()
        # Inject challenge name via JavaScript based on PUBLIC_PORT
        inject_script = f'''<script>
        document.addEventListener('DOMContentLoaded', function() {{
            var h1 = document.querySelector('h1.text-2xl');
            if (h1) h1.textContent = "{CHALLENGE_NAME}";
            document.title = "{CHALLENGE_NAME}";
        }});
        </script></body>'''
